...
> **2026-08-01 status note:** The separate Agent Session HTTP and browser surface is deleted. The direct operator surface is the Run Workspace; this ADR's Agent Workspace and Operator Session transport clauses are historical. Pi remains the disposable Attempt runtime and tool host.

The Agent Workspace was the operator interface at acceptance. An Operator Agent started a Wiki Run by calling the real Pi custom tool `wiki_produce`; ~~that single tool execution owns planning, production, review, and the plan/publication waits~~ (**historical** — under 0035 the tool returns a **StartRun receipt** and WikiRuns owns the durable job). The current browser surface is the Run Workspace and Pi remains an internal Attempt runtime; WikiRuns owns durable control.
