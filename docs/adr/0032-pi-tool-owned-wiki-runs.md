...
> **2026-08-01 status note:** The separate Agent Session HTTP and browser surface was deleted by the v5 hard cut. **2026-08-02:** [ADR 0039](0039-browser-operator-session-and-run-observation.md) restores a deliberately bounded browser Session as a redacted Pi conversation projection; Pi Attempts remain disposable Run workers and the old Agent Workspace clauses remain historical.

The Agent Workspace was the operator interface at acceptance. An Operator Agent started a Wiki Run by calling the real Pi custom tool `wiki_produce`; ~~that single tool execution owns planning, production, review, and the plan/publication waits~~ (**historical** — under 0035 the tool returns a **StartRun receipt** and WikiRuns owns the durable job). Per ADR 0039, the browser now links a bounded Pi conversation to the durable Run Workspace; Pi remains an internal Attempt runtime for Run work, and WikiRuns owns durable control.
