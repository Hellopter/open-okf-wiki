...
| [0039](0039-browser-operator-session-and-run-observation.md) | **Linked browser Operator Session + Run Workspace:** redacted Pi conversation projection; one Session can invoke multiple WikiRuns; Run SSE and Attempt traces remain durable observability authority |
...
2. Prefer **0040 + 0039 + 0038 + 0037 + 0036 + 0035 + 0034 + 0033 + 0032 + 0031 + 0030 + 0021 + 0022 + 0026 + 0028 + 0029** for “how the product is built” (0040 wins on multi-source / monorepo **CoverageUnit** plan gates, freeze CoverageInventory/CoveragePlan, host `assertCoverage`, and `prior_spec` plan revise; 0039 wins on the linked browser Session/Run split and redacted Session transport; 0038 wins on single `repair.N` + unified EvaluationRound re-arm — no hv/review dual paths; 0037 wins on WikiCandidate lineage, EvaluationPolicy budgets, host mechanical autofix, and RepairRequest; 0036 wins on semantic handoff, Run Workspace, and versioned execution contract; 0035 wins on durable Run execution, commands, gates, events, and **Run SSE** fan-out; 0034 wins on deep-module / thin-tool / projection / repair-budget rules; 0033 wins on agent package layering and ports DIP — **current ports `AgentRunner` + `SpecStore` only**; 0032 wins on Pi Session authority and conversation events, not whole-Run tool ownership; 0031 wins on dependency direction; 0030 wins on Pi/tool stack; 0029 wins on no-compat culture). Do **not** reintroduce ProgressSink, GatePort, GraphStore, ReceiptStore, or `run-wiki` shell.
