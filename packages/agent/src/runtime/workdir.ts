...
export function runWorkdirPromptPaths(layout: RunWorkdirLayout): string {
  ...
  return [
    ...
    "  - inputs/         Projected sealed inputs (spec, evidence, defects, prior-wiki, prior-spec, operator-input, coverage, plan-scouts, discovery-map)",
    "  - inputs/operator-input.json  Sealed operator answer when continuing after a gate",
    "  - inputs/prior-spec.json  Sealed prior Spec on plan revise (when bound)",
    "  - inputs/coverage-inventory.json / coverage-plan.json  Host coverage artifacts when sealed",
    "  - inputs/discovery-map.json  Merged discovery authority when sealed (read before synthesizing Spec / writing)",
    "  - inputs/plan-scouts/  Sealed durable plan.scout receipts (read files; do not rely on chat paste)",
