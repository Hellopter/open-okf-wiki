...
export async function assertOrdinaryTree(
  directory: string,
  label: string,
  signal?: AbortSignal,
): Promise<void> {
  ...
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    if (info.isDirectory()) {
      await assertOrdinaryTree(absolutePath, label, signal);
    } else if (!info.isFile()) {
...
export async function makeTreeReadOnly(directory: string, signal?: AbortSignal): Promise<void> {
  signal?.throwIfAborted();
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    signal?.throwIfAborted();
    const absolutePath = path.join(directory, entry.name);
    if (entry.isDirectory()) {
      await makeTreeReadOnly(absolutePath, signal);
    } else {
