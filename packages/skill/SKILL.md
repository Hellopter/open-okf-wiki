...
| `inputs/` | Sealed inputs (read-only): Spec, discovery-map, plan-scouts, evidence receipts, defects, prior Wiki, coverage, and operator input. |
| `wiki/` | Staging Wiki (writable in write roles). |
| `analysis/` | Outputs generated by this Attempt only (writable in write roles). |
...
The Root Agent owns this loop and advances only when the current completion gate holds. During Plan,
write this Attempt's **WikiRunSpec** to `analysis/spec.json`; after it is sealed, every later Attempt
reads `inputs/spec.json`. On large or multi-domain scopes, Root opens the bounded Domain → Leaf
research branch; children investigate and return evidence summaries, while Root keeps Spec synthesis,
Wiki writing, and repair after independent review. Return to an earlier step whenever later evidence
...
4. **Write the Wiki (Spec-bound).** Read sealed `inputs/spec.json` as the **only** page-set
   authority — do not re-plan domains/pages during generate/refresh. Prefer
   `inputs/discovery-map.json` and `inputs/evidence/` for evidence paths. Select only relevant
   files from `skill/templates/{overview,architecture,module,flow,concept}.md`, read them in full,
   and adapt them while writing Spec concept pages under `wiki/` with `write` / `edit` (prefer
   ...
   they support. **Gate:** every critical Spec concept page exists, answers its reader question,
   links to related pages where useful, and is grounded by nearby verified Source Citations.
...
runtime seals each Attempt's receipt and projects it to later Attempts under `inputs/evidence/`.
