export const en = {
  app: {
    brand: "okf-wiki",
    operator: "Operator",
    sidebarAria: "Primary",
    sidebarFoot:
      "Local Web operator UI. Provider secrets stay on this machine, not in workspace.json.",
    expandSidebar: "Expand sidebar",
    collapseSidebar: "Collapse sidebar",
    collapse: "Collapse",
  },
  nav: {
    workspaces: "Workspaces",
    settings: "Settings",
  },
  subnav: {
    aria: "Workspace sections",
    sources: "Sources",
    runs: "Runs",
    wiki: "Wiki",
    settings: "Settings",
    openMenu: "Open workspace sections",
    menuTitle: "Workspace sections",
  },
  commandMenu: {
    title: "Command palette",
    description: "Search navigation and actions",
    placeholder: "Type a command…",
    empty: "No results.",
    navigate: "Navigate",
    appearance: "Appearance",
    themeLight: "Switch to light theme",
    themeDark: "Switch to dark theme",
    themeSystemName: "System",
    themeLightName: "Light",
    themeDarkName: "Dark",
  },
  specReview: {
    audience: "Audience",
    domains: "Domains",
    pages: "Pages",
    optional: "optional",
    openQuestions: "Open questions",
    acceptance: "Acceptance",
    reviewRequired: "review required",
    reviewOptional: "review optional",
    maxRepairRounds: "max repair rounds",
    blocking: "blocking",
    trail: "Notes & replan trail",
    execution: "Execution plan",
    workUnits: "Work units",
    fanOut: "Fan-out",
    reviewLenses: "Review lenses",
    questions: "questions",
    domainPageCounts: "{domains} domains · {pages} pages",
    openQuestionCount: "{count} open questions",
    fanOutValue: "{domains}/{maxDomains} domains · {leaves}/{maxLeaves} leaves",
    workUnitsTruncated: "Showing first {shown} of {total} work units",
    loading: "Loading sealed plan…",
    refreshing: "Refreshing plan to match this gate…",
    loadError: "Could not load the sealed plan. Retry or inspect the attempt transcript.",
    retry: "Retry",
    /** Coverage matrix (assertCoverage projection). */
    coverage: "Coverage",
    coverageUnitId: "Unit",
    coverageKind: "Kind",
    coverageStatus: "Status",
    coveragePageCount: "Pages",
    coverageKindSource: "source",
    coverageKindSurface: "surface",
    coverageStatusCovered: "covered",
    coverageStatusGap: "gap",
    coverageStatusCancelled: "cancelled",
    coverageCounts: "{covered} covered · {gap} gap · {cancelled} cancelled ({total})",
    coverageStopComplete: "complete",
    coverageStopGap: "coverage gap",
    coverageStopNotRequired: "not required",
    coverageNotRequired: "No required coverage units for this plan (light path).",
    coverageGapBlocksApprove:
      "Coverage has gaps. Revise the plan to bind or cancel every required unit before approving.",
    coverageRounds: "{count} re-scout rounds",
    scouts: "Plan scouts",
    scoutsReceipts: "{count} receipts",
    pageSetDiff: "Page set changes",
    pageSetDiffCounts: "+{added} · −{removed} · {retained} retained",
    pageSetAdded: "Added",
    pageSetRemoved: "Removed",
    pageSetRetained: "Retained",
    pageSetNone: "None",
    priorSpec: "Prior Spec",
  },
  runInspector: {
    title: "Wiki Run",
    overviewTab: "Overview",
    planTab: "Plan",
    graphTab: "Workflow",
    attemptsTab: "Attempts",
    noAttempts: "No attempts recorded for this run.",
    open: "Open run details",
    close: "Close run details",
    noSpec: "No plan recorded for this run.",
    noGraph: "No run graph recorded for this run.",
  },
  locale: {
    label: "Language",
    en: "English",
    zh: "中文",
    switchTo: "Switch language",
  },
  common: {
    loading: "Loading…",
    save: "Save changes",
    saving: "Saving…",
    saved: "Saved",
    cancel: "Cancel",
    create: "Create",
    delete: "Delete",
    deleting: "Deleting…",
    removing: "Removing…",
    on: "On",
    off: "Off",
    optional: "optional",
    actions: "Actions",
    error: "Error",
    dismiss: "Dismiss",
    confirm: "Confirm",
    name: "Name",
    path: "Path",
    id: "ID",
  },
  workspaces: {
    title: "Workspaces",
    descriptionBefore:
      "A Workspace is a local project: Git sources, selected model, and wiki output path. Models and credentials are managed in ",
    descriptionAfter: ".",
    settingsLink: "Settings",
    create: "Create",
    cancel: "Cancel",
    createTitle: "Create workspace",
    nameLabel: "Name",
    namePlaceholder: "My project",
    rootLabel: "Root path (absolute)",
    rootPlaceholder: "D:/src/app",
    rootHint:
      "Absolute path on this machine. The server creates the directory and .okf-wiki/ if needed.",
    createSubmit: "Create workspace",
    creating: "Creating…",
    loading: "Loading workspaces…",
    emptyTitle: "No workspaces yet",
    emptyDescription:
      "Create a workspace with an absolute root path. Then add local Git checkouts as sources.",
    checklistModels: "Configure models in",
    checklistSources: "Add sources: link a local Git checkout or clone into the workspace",
    checklistRun: "Run Wiki generation with HITL publish",
    colName: "Name",
    colRoot: "Root path",
    colSources: "Sources",
    colLastOpened: "Last opened",
    sourceCountLabel: "{n} sources",
    rowMenu: "Workspace actions",
    colActions: "Actions",
    delete: "Delete",
    deleteConfirmTitle: "Delete workspace?",
    deleteConfirmBody:
      "Remove “{name}” from the recent list. Project files on disk are kept unless you also remove meta.",
    deleteMetaLabel: "Also delete .okf-wiki/ under the root (runs, sessions, staging)",
    deleteSubmit: "Delete workspace",
    deleting: "Deleting…",
  },

  settings: {
    breadcrumbWorkspaces: "Workspaces",
    breadcrumbSettings: "Settings",
    title: "Workspace settings",
    descriptionPrefix: "Models & credentials →",
    descriptionLink: "Settings",
    descriptionSuffix: ".",
    loading: "Loading settings…",
    tabGeneral: "General",
    tabSkill: "Skill",
    tabDanger: "Danger",
    generalCardDescription: "Identity, session budgets, orchestration, and role models.",
    sectionIdentity: "Identity",
    sectionSession: "Session & timeouts",
    maxActiveRuns: "Active runs",
    maxConcurrentAttempts: "Concurrent attempts",
    metaTitle: "Workspace meta",
    metaDescription: "Read-only paths and identifiers for this workspace.",
    name: "Name",
    publicationPath: "Publication path (absolute)",
    planConfirm: "Plan confirm",
    planConfirmHint:
      "Pause interactive runs for operator approval of the Wiki Spec (domains + pages).",
    wikiLanguage: "Wiki language",
    wikiLanguageHint:
      "Language used for generated Wiki page titles and body (not the operator UI).",
    contextTargetTokens: "Context target (tokens)",
    contextTargetTokensHint:
      "Operational budget for Pi auto-compaction near the context target. Leave blank to use 85% of the selected model’s max context when that is configured.",
    contextTargetTokensPlaceholder: "e.g. 100000",
    contextTargetDerived: "Currently derived from model max: {n} tokens (85%).",
    requestTimeoutSeconds: "Agent session timeout (seconds)",
    requestTimeoutSecondsHint:
      "Wall-clock budget for each Wiki Run child attempt (plan, research, write, review). Long plans often need 600–1800s. Too low aborts with “workspace request timeout” while the model is still working. Saves apply to Runs started after save — not in-flight attempts.",
    requestTimeoutSecondsPlaceholder: "e.g. 600",
    gateTimeoutSeconds: "Operator gate timeout (seconds)",
    gateTimeoutSecondsHint:
      "When greater than 0, open plan/publication gates auto-deny after this many seconds. 0 disables. Applies to gates opened on Runs that see the saved workspace config.",
    gateTimeoutSecondsPlaceholder: "0 = disabled; e.g. 86400 for 24h",
    retryTitle: "Provider transport retry",
    retryHint:
      "Pi auto-retries transient provider errors (429/5xx/network) inside the same child attempt. Does not reopen sessions or retry context overflow. Waits count against the session timeout above. Saves apply to subsequent Runs only.",
    retryEnabled: "Enable auto-retry",
    retryMaxRetries: "Max retries",
    retryMaxRetriesHint:
      "Extra attempts after the first call (default 2 → 3 total). Same-session only.",
    retryBaseDelayMs: "Base delay (ms)",
    retryBaseDelayMsHint: "Exponential backoff base (2s → 4s → 8s when default 2000).",
    providerMaxRetries: "SDK retries",
    providerMaxRetriesHint:
      "Nested provider/SDK retries. Keep 0 unless a gateway needs it (can stack with auto-retry).",
    providerMaxRetryDelayMs: "Max Retry-After (ms)",
    providerMaxRetryDelayMsHint:
      "If the server asks to wait longer than this, fail immediately instead of sleeping.",
    orchestrationTitle: "Supervisor tree budgets",
    orchestrationHint:
      "Run Boundary budgets for plan scouts, Domain/Leaf research, and multi-lens review. Light-path defaults (0 scouts, 1 lens); raise for large or multi-entry codebases.",
    maxDomainFanOut: "Max domains",
    maxLeafFanOut: "Max leaves / domain",
    maxLeafFanOutHint:
      "Topology only: max questions/leaves materialized per domain. Does not set the leaf concurrency pool.",
    planScoutCount: "Plan scouts",
    planScoutCountHint:
      "Parallel read-only scouts (entry / layout / tests / risks) before the Spec synthesizer. Default 0 (light path: inventory + one planner); raise for large or multi-entry repos.",
    reviewCouncilSize: "Review council size",
    reviewCouncilSizeHint:
      "Independent review lenses (grounding, coverage, consistency, …). Default 1 (light path); raise when extra lenses earn unique defects.",
    reviewConcurrency: "Review concurrency",
    reviewConcurrencyHint: "How many council members run at once (defaults to council size).",
    domainConcurrency: "Domain concurrency",
    domainConcurrencyHint:
      "How many domains research in parallel. Also scales the shared leaf pool with leaf concurrency.",
    leafConcurrency: "Leaf concurrency",
    leafConcurrencyHint:
      "Per-domain leaf parallel width. Total leaf slots = domain concurrency × min(leaf concurrency, max leaves / domain).",
    roleModelsTitle: "Role models (hybrid economics)",
    roleModelsHint:
      "Optional: frontier planner + cheaper workers/scouts + separate reviewers. Empty roles fall back to the workspace default model above. Pi auto-compacts each role session near the context target.",
    rolePlanner: "Planner (Spec synthesizer)",
    roleWorker: "Worker (research / scouts)",
    roleWriter: "Writer",
    langEn: "English",
    langZh: "Chinese (Simplified)",
    save: "Save changes",
    saving: "Saving…",
    saved: "Saved",
    orphanModel: "Previous model id {id} is no longer in Settings. Pick a configured model above.",
    skillTitle: "Producer Skill",
    skillDescription:
      "Global method package for wiki generation. Defaults follow Agent Skills paths: project `.agents/skills`, then user `~/.agents/skills` (Settings), then the product package. Create a workspace fork to customize templates and guidance; each Wiki Run freezes the skill content digest.",
    skillKind: "Kind",
    skillDigest: "Digest",
    skillPath: "Path",
    skillName: "Name",
    skillUnavailable: "Skill info unavailable.",
    skillFork: "Create / refresh fork",
    skillWorking: "Working…",
    skillBundled: "Use default (home/package)",
    skillLoadFile: "Load file",
    skillSaveFile: "Save file",
    skillFileLabel: "Skill file (fork editor)",
    skillFileContentLabel: "Skill file contents",
    skillListRoot: "list root",
    skillFiles: "Files:",
    rootPath: "Root path",
    selectedModelId: "Selected model id",
    modelProfile: "Model profile",
    dangerTitle: "Danger zone",
    dangerDescription:
      "Remove this workspace from the app index. Optionally delete the .okf-wiki meta directory (not the whole project tree).",
    deleteWorkspace: "Delete workspace",
    deleteConfirmTitle: "Delete workspace?",
    deleteConfirm:
      "Delete workspace “{name}” from the recent list? This cannot be undone from the UI.",
    deleteMeta: "Also delete .okf-wiki/",
    skillForked: "Skill fork ready",
    skillSaved: "Skill file saved",
  },
  sources: {
    probeNotGit: "Not a git checkout",
    probeNotGitError: "Not git: {error}",
    probeDetached: "detached",
    probeDirty: "dirty",
    probeClean: "clean",
    breadcrumbWorkspaces: "Workspaces",
    breadcrumbSources: "Sources",
    title: "Sources",
    description:
      "Link an existing local Git checkout (absolute path, may be outside the workspace root) or clone a remote into this workspace under sources/<id>. Clone uses host git credentials; tokens are never stored in workspace config.",
    loading: "Loading sources…",
    registered: "Registered sources",
    probeAll: "Probe all",
    probing: "Probing…",
    empty: "No sources yet. Add an absolute path to an existing local Git checkout.",
    colId: "ID",
    colOrigin: "Origin",
    colPath: "Path",
    colProbe: "Probe",
    colIgnores: "Ignores",
    originPath: "path",
    originClone: "clone",
    delete: "Delete",
    removing: "Removing…",
    deleteConfirmTitle: "Delete source?",
    deleteConfirmBody:
      "Remove source “{id}” from this workspace? Linked paths are not deleted from disk; cloned trees under the workspace may remain.",
    deleteSubmit: "Delete source",
    linkTitle: "Link existing path",
    pathLabel: "Path (absolute)",
    pathPlaceholder: "D:/src/repo",
    sourceIdLabel: "Source id",
    sourceIdOptional: "(optional slug)",
    sourceIdPlaceholder: "app",
    sourceIdHint: "Lowercase slug. Leave blank to derive from the path.",
    addSource: "Add source",
    adding: "Adding…",
    cloneTitle: "Clone into workspace",
    cloneHint: "Clones into {root}/sources/<id>.",
    remoteUrl: "Remote URL",
    cloneId: "Source id",
    cloneRef: "Ref (optional)",
    cloneSubmit: "Clone source",
    cloning: "Cloning…",
    ignoreTitle: "Ignore rules",
    ignoreDescription:
      "Repository-relative POSIX globs. Default product ignores (node_modules, build, caches, …) apply unless disabled. User patterns are always additive. Tests are not ignored by default.",
    applyDefaults: "Apply default source ignores",
    ignorePatterns: "Custom ignore patterns (one per line)",
    ignorePlaceholder: "src/test/**\n**/*Test.java",
    presets: "Presets",
    presetJava: "Java tests",
    presetJs: "JS/TS tests",
    presetPython: "Python tests",
    saveIgnores: "Save ignore rules",
    savingIgnores: "Saving…",
    editIgnores: "Edit ignores",
    closeEditor: "Close",
    defaultsOn: "defaults on",
    defaultsOff: "defaults off",
    customCount: "{n} custom",
  },
  globalSettings: {
    title: "Settings",
    description:
      "Machine-local model profiles and credentials. Workspace config never stores API keys.",
    loading: "Loading settings…",
    refresh: "Refresh",
    addModel: "Add model",
    addProvider: "Add provider",
    providerCreateTitle: "New provider",
    providerEditTitle: "Edit provider",
    providerName: "Provider name",
    providerNamePlaceholder: "e.g. My gateway",
    statusProviderAdded: "Provider added",
    statusProviderUpdated: "Provider updated",
    statusProviderDeleted: "Provider deleted",
    providerDeleteConfirmTitle: "Delete provider?",
    providerDeleteConfirmBody:
      "Delete provider {name} and all models under it? Workspaces referencing them must pick another model.",
    tabModels: "Models",
    tabApp: "App",
    tabDiagnostics: "Diagnostics",
    skillsTitle: "Producer Skill (Agent Skills)",
    skillsDescription:
      "Portable Agent Skills layout (same as Codex/Grok): project `.agents/skills`, then user `~/.agents/skills` when enabled below (seeded from the product package on first use). Disable user skills to use the package-embedded skill only when no project skill exists.",
    loadHomeSkills: "Load skills from user ~/.agents/skills",
    loadHomeSkillsHint:
      "When off, skip ~/.agents/skills and fall back to package assets (project .agents/skills still wins).",
    homeSkillsPath: "User skills path",
    workspaceSkillsPath: "Project skills path",
    skillsSaving: "Saving…",
    skillsSaved: "Saved",
    appSettingsUnavailable: "App settings unavailable.",
    modelsTitle: "Providers & models",
    providersHint:
      "OpenCode-style: one gateway (base URL + key + headers) can host many models. User-Agent defaults to node for gateways that block the OpenAI SDK fingerprint.",
    modelsCount: "{n} models",
    defaultSet: "default set",
    noDefault: "no default",
    modelsEmpty:
      "No providers yet. Add an OpenAI-compatible gateway + model, then select it when creating a workspace.",
    providerModelsEmpty: "No models under this provider yet. Use Add model to attach one.",
    addModelUnderProvider: "Add model",
    colName: "Name",
    colModelId: "Model id",
    colShape: "Shape",
    colBaseUrl: "Base URL",
    colKey: "Key",
    colMaxContext: "Max ctx",
    colActions: "Actions",
    defaultBadge: "default",
    setDefault: "Set default",
    edit: "Edit",
    delete: "Delete",
    envFallback: "Env fallback: OPENAI_BASE_URL={base} · OPENAI_API_KEY={key}",
    envSet: "set",
    envUnset: "unset",
    keySet: "set",
    editorCreateTitle: "Add model",
    editorEditTitle: "Edit model / provider connection",
    displayName: "Display name",
    displayNamePlaceholder: "Corp GPT-4o",
    modelIdLabel: "Model id",
    modelIdPlaceholder: "openai/my-served-model",
    modelIdHint:
      "Served model id for the OpenAI-compatible gateway (e.g. openai/gpt-4o or bare deployment name). Only OpenAI-compatible providers are supported.",
    baseUrl: "Base URL",
    baseUrlPlaceholder: "https://gateway.example.com/v1",
    apiKey: "API key",
    apiKeyKeepPlaceholder: "Leave blank to keep stored key",
    apiKeyPlaceholder: "sk-… or gateway token",
    clearApiKey: "Clear stored API key",
    apiShape: "API shape",
    shapeCompletions: "Chat Completions",
    shapeResponses: "Responses",
    userAgent: "User-Agent (provider)",
    userAgentHint:
      "Sent on agent requests (Pi/OpenAI SDK). Use node if the gateway blocks OpenAI/JS. Shared by all models on this endpoint.",
    supportsDeveloperRole: "Use OpenAI developer role",
    supportsDeveloperRoleHint:
      "Off by default. Enable only for official OpenAI (or fully compatible) endpoints. Third-party gateways often reject role “developer” with 400.",
    developerRoleOn: "developer role on",
    addingUnderProvider: "Adding under provider {id}",
    maxContextTokens: "Max context (tokens)",
    maxContextTokensHint:
      "Provider hard context window for this model. Wiki Runs auto-compact when usage reaches about 85% of this value (or the workspace context target).",
    maxContextTokensPlaceholder: "e.g. 128000",
    saving: "Saving…",
    saveCreate: "Add model",
    saveEdit: "Save changes",
    testing: "Testing…",
    testConnection: "Test connection",
    testOk: "reachable",
    testFail: "failed",
    healthTitle: "API connection",
    apiBase: "API base",
    apiBaseSameOrigin: "(same origin)",
    health: "Health",
    healthNotChecked: "Not checked yet",
    healthOk: "ok · {service}",
    healthNotOk: "not ok",
    checking: "Checking…",
    runHealthCheck: "Run health check",
    doctorTitle: "Doctor",
    status: "Status",
    statusOk: "ok",
    statusNotOk: "not ok",
    node: "Node",
    platform: "Platform",
    git: "Git",
    gitAvailable: "available",
    gitUnavailable: "unavailable",
    doctorModels: "Models",
    doctorModelsConfigured: "{n} configured",
    doctorNoCredentials: "no credentials",
    deleteConfirmTitle: "Delete model?",
    deleteConfirmBody:
      "Delete model “{name}”? Workspaces that selected it keep the last known model id.",
    deleteSubmit: "Delete model",
    deleting: "Deleting…",
    statusModelAdded: "Model added",
    statusModelUpdated: "Model updated",
    statusModelDeleted: "Model deleted",
    statusDefault: "Default: {name}",
  },
  wiki: {
    breadcrumbWorkspaces: "Workspaces",
    breadcrumb: "Wiki",
    title: "Published Wiki",
    description: "Read-only browse of pages under the workspace publication path.",
    loading: "Loading published wiki…",
    emptyTitle: "Not published yet",
    emptyDescription:
      "No markdown pages were found under the publication path. Start a Run and approve publication to populate this view.",
    goToRuns: "Open Runs",
    pages: "Pages",
    pagesAria: "Wiki pages",
    loadingPage: "Loading page…",
    selectPage: "Select a page from the list.",
    graphView: "Graph",
    listView: "Pages",
    graphAria: "Wiki cross-link graph",
    graphEmpty: "No cross-links between pages yet.",
    backlinks: "Referenced by",
    generatedAt: "Generated",
    /** Pages not listed in any index.md (appended at the end of the TOC). */
    unlisted: "Unlisted",
    trustTier: {
      unverified: "Unverified",
      machineConfirmed: "Machine-confirmed",
      humanReviewed: "Human-reviewed",
    },
  },
  planConfirm: {
    title: "Proposed page plan",
    notes: "Notes",
    pagesLabel: "Pages",
    approve: "Approve plan & write",
    working: "Working…",
    decline: "Decline plan",
    revise: "Request changes",
    reviseHint: "Describe how the page plan should change.",
    reviseLabel: "Revision feedback",
    revisePlaceholder: "e.g. add concepts.md, drop architecture.md",
    reviseSubmit: "Submit revision",
    fullscreen: "View plan fullscreen",
    /** Session decision chip labels (ids: approve | revise | deny). */
    chipWrite: "Write {n} page(s)",
    chipRevise: "Request changes",
    chipDeny: "Reject this plan",
    chipPublish: "Publish staged wiki",
    chipKeepStaging: "Keep staging only",
  },
  /** Operator fix gate (kind "fix") — defect / quality HITL. */
  fixConfirm: {
    title: "Review defects",
    working: "Working…",
    /** Accept current staging / mark reported issues as non-blocking. */
    pass: "Pass (not a problem)",
    /** Request automated or agent repair for reported defects. */
    fix: "Fix",
    revise: "Request changes",
    reviseLabel: "Revision notes",
    revisePlaceholder: "e.g. keep overview.md as-is; repair modules.md citations only",
    reviseSubmit: "Submit notes",
    deny: "Deny",
    clean: "Clean",
    defectCount: "{n} defect(s)",
    suggestedFix: "Suggested fix",
    noDefectsHint: "No sealed defects list yet — use Pass, Fix, Revise, or Deny.",
  },
  /** Durable operator_input gate — answer a fact question; does not resume old Pi worker. */
  operatorInput: {
    title: "Operator input needed",
    answerLabel: "Your answer",
    answerPlaceholder: "Provide the missing fact or decision…",
    submit: "Submit answer",
    working: "Working…",
    answerRequired: "Enter a non-empty answer before submitting.",
    questionFallback: "The run is waiting for an operator answer.",
  },
  modelSelect: {
    label: "Model",
    emptyBefore: "No models configured. ",
    emptyLink: "Add a model in Settings",
    emptyAfter: " first.",
    selectPlaceholder: "Select a model…",
    defaultSuffix: "(default)",
    hintBefore: "Models are managed in ",
    hintLink: "Settings",
    hintAfter: ". Base URL and API key are not set per workspace.",
  },
  errorBanner: {
    title: "Request failed",
    dismiss: "Dismiss",
    unknown: "Unknown error",
  },
  validation: {
    maxContextTokens: "maxContextTokens must be a positive integer",
    contextTargetTokens: "contextTargetTokens must be a positive integer",
    requestTimeoutSeconds: "requestTimeoutSeconds must be between 1 and 86400",
    retryMaxRetries: "retry.maxRetries must be an integer from 0 to 10",
    retryBaseDelayMs: "retry.baseDelayMs must be an integer from 100 to 60000",
    providerMaxRetries: "retry.provider.maxRetries must be an integer from 0 to 5",
    providerMaxRetryDelayMs: "retry.provider.maxRetryDelayMs must be an integer from 0 to 600000",
    gateTimeoutSeconds: "gateTimeoutSeconds must be an integer from 0 to 604800",
    lineRange: "Choose a valid line range.",
    runCapacity: "Set workspace Run capacity before starting a Run.",
  },
  runStatus: {
    queued: "Queued",
    running: "Running",
    waiting_for_operator: "Awaiting your action",
    cancelling: "Cancelling",
    published: "Published",
    failed: "Failed",
    cancelled: "Cancelled",
    completed: "Completed",
    awaiting: "Waiting",
    pending: "Pending",
    idle: "Not started",
    skipped: "Skipped",
    open: "Open",
    resolved: "Resolved",
    withdrawn: "Withdrawn",
    awaiting_plan: "Awaiting plan",
    awaiting_publication: "Awaiting publication",
    awaiting_fix: "Awaiting fix review",
    awaiting_operator_input: "Awaiting operator input",
    publication_declined: "Publication declined",
  },
  workbench: {
    sessions: "Sessions",
    runs: "Runs",
    activity: "Workspace activity",
    newSession: "New session",
    deleteSession: "Delete session",
    deleteSessionTitle: "Delete this session?",
    deleteSessionDescription:
      "Remove the saved conversation “{title}”. Wiki Runs created from it are kept.",
    untitledSession: "Untitled session",
    noRuns: "No Wiki Runs yet.",
    conversation: "Conversation",
    activeRun: "Active Run",
    refresh: "Refresh workspace",
    openActivity: "Open activity",
    connection: "Connection",
    agent: "Wiki Agent",
    startConversation: "Start a conversation or ask the agent to generate a Wiki.",
    promptIdle: "Ask about the workspace, or use /wiki to generate a Wiki",
    promptBusy: "Steer the active agent",
    send: "Send message",
    stop: "Stop agent",
    modelPicker: {
      ariaLabel: "Session model",
      menuLabel: "Switch model",
      empty: "No models",
      emptySettingsTitle: "Add models in Settings",
      /** Menu third line; `{n}` is a compact token count (e.g. 128k). */
      maxContext: "{n} ctx",
      busyTitle: "Wait for the current turn before switching models",
    },
    context: {
      meterAria: "Context fill",
      window: "Window",
      target: "Target",
      tokens: "Tokens",
      phase: "Phase",
      compactHint: "Type /compact to free context space, or /compact stop while the agent is busy",
      insertCompact: "Insert /compact",
      phases: {
        normal: "Normal",
        approaching_target: "Approaching target",
        at_target: "At target",
        compacting: "Compacting",
        unknown: "Unknown",
      },
      /** Attempt observation: read-only model chip aria. */
      modelAria: "Attempt model",
      /** Side-note fragments; `{n}` is a compact token/tool count. */
      inTokens: "in {n}",
      outTokens: "out {n}",
      toolCalls: "tools {n}",
      /** Graph node micro-dot aria when fill percent is known. */
      graphFillAria: "Context fill {percent}%",
      graphFillAriaUnknown: "Context usage",
    },
    slash: {
      listLabel: "Slash commands",
    },
    pauseRun: "Pause Run",
    pauseRunHint: "Soft-stop: the Run can be resumed later.",
    resumeRun: "Resume",
    cancelRun: "Cancel Run",
    cancelRunConfirm:
      "Cancel this Run? Cancellation is terminal — you cannot resume a cancelled Run.",
    retryNode: "Retry",
    rerunNode: "Rerun",
    retryNodeHint: "Retry the same generation with the same sealed inputs.",
    rerunNodeHint: "Start a new generation and invalidate downstream consumers.",
    rerunNodeConfirmTitle: "Rerun this node?",
    rerunNodeConfirm:
      "Rerun bumps the node generation. Downstream work that depended on this node may be invalidated.",
    rerunNodeConfirmConsumers:
      "Downstream nodes already progressed. Rerunning will invalidate consumers and re-queue lineage work.",
    rerunFeedbackPlaceholder: "Optional guidance for the new generation",
    recoveryBannerTitle: "Recovery needed",
    recoveryBanner:
      "{count} node(s) need attention (including “{node}”). Open a failed node to Retry or Rerun.",
    recoveryBannerWithReason:
      "{count} node(s) need attention (including “{node}”): {reason}. Open a failed node to Retry or Rerun.",
    recoveryBannerGeneric:
      "This Run has failed or interrupted work. Open a node to Retry or Rerun.",
    recoveryOpenNode: "Open {node}",
    attemptErrorTitle: "Attempt failed",
    /** Clearer title when failureClass/error indicates context capacity. */
    capacityFailureTitle: "Context capacity exceeded",
    capacityFailureHint:
      "This attempt ran out of model context. Retry with a smaller task, or Rerun after adjusting scope.",
    noAutoRetryHint:
      "This failure will not auto-retry. Use Retry or Rerun, or fix configuration and start a new Run.",
    cannotRetryReason: {
      runPublished: "Cannot retry on a published Run; start a new Run.",
      runCancelled: "Cannot retry on a cancelled Run.",
      nodeNotFound: "Node is not in this Run snapshot.",
      nodeNotFailed: "Only failed nodes can be retried.",
      nodeCancelled: "This node was cancelled.",
      nodeInvalidated: "This node was invalidated; use Rerun on an upstream node.",
      attemptMissing: "No attempt is recorded for this node.",
      attemptNotRetryable: "The last attempt is not failed or interrupted.",
      freezeNotPinned: "Cannot retry freeze before inputs are pinned; start a new Run.",
      hasConsumers: "Downstream already consumed this output; use Rerun instead of Retry.",
    },
    cannotRerunReason: {
      runPublished: "Cannot rerun on a published Run; start a new Run.",
      runCancelled: "Cannot rerun on a cancelled Run.",
      nodeNotFound: "Node is not in this Run snapshot.",
      repairNode: "Cannot rerun a repair node; retry a failed repair or schedule a new one.",
      planMaterialized:
        "Cannot rerun plan after execution topology exists; start a new Run instead.",
    },
    plan: "Plan",
    execution: "Execution",
    candidates: "Candidates",
    attempts: "Attempts",
    trace: "Trace",
    planPending: "Plan not sealed yet.",
    selectAttempt: "Select an attempt",
    waitingTool: "Waiting for tool output",
    toolCalled: "called",
    toolPending: "Pending",
    toolRunning: "Running",
    toolCompleted: "completed",
    toolFailed: "failed",
    toolInput: "Input",
    toolOutput: "Output",
    toolError: "Error",
    rawInput: "Raw input",
    rawOutput: "Raw output",

    sessionRuns: "Wiki Runs from this session",
    openRun: "Open Run",
    decisionTitle: "{kind} requires a decision",
    decisionFallback: "This Run is waiting for operator input.",
    answerPlaceholder: "Answer for this Run",
    guidancePlaceholder: "Optional guidance",
    sendAnswer: "Send answer",
    acceptCandidate: "Accept candidate",
    repairAutomatically: "Repair automatically",
    approve: "Approve",
    revise: "Revise",
    decline: "Decline",
    runCommandAccepted: "Run command accepted",
    sessionBusy: "The Session already has an active turn",
    noActiveTurn: "There is no active turn to redirect",
    commandRejected: "Session command was not accepted",
    stopFailed: "Unable to stop Session",
    connectionStates: {
      connecting: "Connecting",
      live: "Connected",
      reconnecting: "Reconnecting",
      offline: "Offline",
    },
    runStates: {
      queued: "Queued",
      running: "Running",
      pausing: "Pausing",
      paused: "Paused",
      waiting_for_operator: "Waiting for input",
      failed: "Failed",
      publication_declined: "Publication declined",
      completed_unpublished: "Ready to publish",
      published: "Published",
      cancelling: "Cancelling",
      cancelled: "Cancelled",
    },
    nodeStates: {
      blocked: "Blocked",
      ready: "Ready",
      running: "Running",
      waiting: "Waiting",
      succeeded: "Completed",
      failed: "Failed",
      invalidated: "Invalidated",
      cancelled: "Cancelled",
    },
    gateKinds: {
      plan: "Plan",
      operator_input: "Operator input",
      publication: "Publication",
      fix: "Quality repair",
    },
    traceKinds: {
      input: "Input",
      assistant: "Assistant",
      tool_call: "Tool call",
      tool_result: "Tool result",
      terminal: "Completion",
      truncated: "Trace limit",
    },
    autoRepair: "Automatic repair",
    semanticRepairRounds: "Review repair rounds",
    mechanicalRepairRounds: "Validation repair rounds",
    reviewEnabled: "Review required",
    recheck: "re-evaluate",
    parallelResearch: "Parallel research",
    qualityLoop: "Review, repair, and publication",
    stagePlan: "Plan",
    stageResearch: "Parallel research",
    stageSynthesis: "Synthesis",
    stageQuality: "Quality loop",
    stagePublication: "Publication",
    planAdapt: "Plan adaptation",
    independentResearch: "Independent research",
    backToOverview: "All stages",
    noStageNodes: "No nodes were created for this stage.",
    fanOut: "fan out",
    fanIn: "join",
    controlEdge: "gate",
    domainLeafCount: "{count} leaves",
    expandDomain: "Show leaves",
    collapseDomain: "Hide leaves",
    expandAllDomains: "Expand all",
    collapseAllDomains: "Collapse all",
    backToGraph: "Execution graph",
    nodeActivity: "Activity",
    noAttemptStarted: "This node has not started an attempt yet.",
    noTrace: "No messages or tool events have been recorded yet.",
    attemptInput: "Attempt input",
    attemptHistory: "Attempt history",
    followLive: "Follow live",
    loadEarlier: "Load earlier events",
    rawEvents: "Raw events",
    thinking: "Thinking",
    thought: "Thought",
    thinkingElapsed: "{seconds}s",
    generating: "Generating…",
    toolCallsSummary: "{count} tool calls",
    toolCallsWithMessages: "{count} tool calls, {messages} messages",
    moreFiles: "+{count} more",

    toolNames: {
      wiki_produce: "Generate wiki",
    },
    copy: "Copy",
    copied: "Copied",
    technicalDetails: "Technical details",
  },
  loading: {
    default: "Loading…",
  },
} as const;

/** Deep string map with the same shape as the English catalog. */
export type MessageTree = DeepStringify<typeof en>;

type DeepStringify<T> = T extends string
  ? string
  : T extends readonly (infer U)[]
    ? DeepStringify<U>[]
    : T extends object
      ? { [K in keyof T]: DeepStringify<T[K]> }
      : T;
