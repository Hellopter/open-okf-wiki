...
function setup() {
  ...
  writeJson(path.join(workdir, "inputs", "inventory.json"), {
    coverageUnits: [
      { id: "api", required: true },
      { id: "api::web", required: true },
...
describe("artifact checkpoint publishing", () => {
  it("publishes a minimal discover-to-plan chain and detects later artifact mutation", () => {
    ...
    const discover = publish(root, run, "discover", [
    ...
    assert.deepEqual(Object.keys(discover.artifacts[0]).sort(), ["coverageUnitIds", "digest", "id", "path", "type"]);
