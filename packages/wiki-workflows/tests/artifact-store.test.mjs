...
test("writes and resumes content-addressed Markdown handoffs through the manifest", async (t) => {
  ...
  const input = { runId: "run-1", nodeId: "research-1", attempt: 1, kind: "research", content: "# Finding\n" };
...
test("rejects unsafe, oversized, forged, and symlinked handoffs", async (t) => {
  const { workspace, store } = await fixture(t);
  await assert.rejects(() => store.write({ runId: "../bad", nodeId: "n", attempt: 1, kind: "research", content: "x" }), /Invalid Wiki handoff run ID/);
  await assert.rejects(() => store.write({ runId: "run", nodeId: "n", attempt: 1, kind: "research", content: "x".repeat(MAX_WIKI_RESEARCH_ARTIFACT_BYTES + 1) }), /262144-byte limit/);
  const ref = await store.write({ runId: "run", nodeId: "n", attempt: 1, kind: "research", content: "valid\n" });
