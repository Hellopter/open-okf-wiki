...
async function workspaceWithSources(t) {
  const parent = await mkdtemp(path.join(os.tmpdir(), "okf-wiki-executor-v5-"));
...
function fakeSession(activeTools) {
  ...
  return {
    ...
    getActiveToolNames: () => activeTools ?? ["read", "grep", "find", "ls", "edit", "write", "wiki_write_handoff", "wiki_submit_synthesis", "wiki_submit_review"],
...
function request(cwd, kind, role) {
  const artifactWritePath = role === "writer" ? undefined : `.okf-wiki/runs/run/${kind}-node/attempt-1/${kind === "research" ? "research.md" : `${kind}.json`}`;
  return {
    runId: "run",
    node: node(kind),
    cwd,
...
async function writeAndSubmit(tools, name, content) {
  ...
  return await tools.find((tool) => tool.name === name).execute("submit", {
    artifactPath: name === "wiki_submit_synthesis"
...
function finalizedSpec() {
  return {
    domains: [
      { id: "overview", title: "Overview", purpose: "Orient readers", pages: [{ pageType: "overview", path: "overview/overview.md", title: "Overview", purpose: "Orient readers", researchScopeIds: [] }] },
      { id: "domain", title: "Domain", purpose: "Explain the domain", pages: [{ pageType: "module", path: "domain/page.md", title: "Page", purpose: "Explain the page", researchScopeIds: ["api"] }] },
    ],
  };
}
...
test("synthesis submission accepts the page-level contract and optional coordination arrays", async (t) => {
  ...
  execution.artifactPaths = [".okf-wiki/runs/run/research-node/attempt-1/research.md"];
  await mkdir(path.dirname(path.join(cwd, execution.artifactPaths[0])), { recursive: true });
  await writeFile(path.join(cwd, execution.artifactPaths[0]), "## Verified Evidence\n- API.\n");
  let tools;
  ...
  assert.match(submit.promptGuidelines[0], /researchScopeIds/);
...
test("writer tools expose no deletion and enforce one exact write page", async (t) => {
  const cwd = await workspaceWithSources(t);
  await mkdir(path.join(cwd, "wiki/domain"), { recursive: true });
  ...
  const executor = new PiAgentExecutor({ createSession: async (options) => {
    tools = options.customTools;
    return { session: fakeSession(options.tools) };
  } });
...
test("research Markdown is limited to 64 KiB while JSON control artifacts allow 256 KiB", async (t) => {
  const cwd = await workspaceWithSources(t);
  ...
  session.prompt = async () => {
    ...
    await tools.find((tool) => tool.name === "wiki_write_handoff").execute("valid", { content: "# Receipt\n" });
  };
  const executor = new PiAgentExecutor({ createSession: async (options) => {
    tools = options.customTools;
    return { session };
  } });
  assert.equal((await executor.execute(execution)).result, "# Receipt\n");
...
test("missing the required submission fails with a classified protocol error", async (t) => {
  ...
  execution.artifactPaths = [".okf-wiki/runs/run/research-node/attempt-1/research.md"];
  await mkdir(path.dirname(path.join(cwd, execution.artifactPaths[0])), { recursive: true });
  await writeFile(path.join(cwd, execution.artifactPaths[0]), "# Research\n");
  const session = fakeSession();
  const executor = new PiAgentExecutor({ createSession: async (options) => {
    session.getActiveToolNames = () => options.tools;
    return { session };
