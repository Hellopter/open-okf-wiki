...
test("root declarations expose the complete caller type closure without internal records", async () => {
  ...
  for (const name of ["WikiProducer", "WikiRunHandle", "WikiRunUpdate", "WikiRunView", "WikiRunEvent", "WikiAgentInspection", "WikiAgentOutcome", "WikiRunWarning", "WikiActivityPage"]) {
...
test("npm pack exposes a self-contained public declaration graph to a real TypeScript consumer", async (t) => {
  ...
  await writeFile(path.join(consumer, "consumer.ts"), [
    ...
    "  if (event.type === 'telemetry' && event.phase === 'observability_health') return event.status;",
    "  if (event.type === 'telemetry') return event.phase;",
