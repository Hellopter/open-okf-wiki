...
test("formats absolute timestamps in the system timezone", () => {
  const script = [
    ...
    'process.stdout.write(formatLocalDateTime("2026-08-12T00:01:02.000Z"));',
  ].join("\n");
  ...
  assert.equal(output, "Aug 12, 2026, 8:01:02 AM");
});

test("preserves invalid timestamp text", async () => {
  const { formatLocalDateTime } = await import("../dist/time-format.js");
